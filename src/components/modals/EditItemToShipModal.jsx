...
const itemOptions = itemData.map(item => ({
    value: item.name, 
    label: item.name 
}));

function EditItemToShipModal({ isOpen, onClose, onUpdate, initialData }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!data.itemName || !data.quantity) return alert("Please fill in all fields");
        ...
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Quantity (KG)</label>
...
export default EditItemToShipModal;