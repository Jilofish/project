...
function AddProductModal({ isOpen, onClose, supplierOptions }) {
    ...
    // --- NEW STATE FOR SUPPLIER PRICING ---
    const [isPriceModalOpen, setIsPriceModalOpen] = useState(false);
    const [supplierPrices, setSupplierPrices] = useState([]); // Array of { id, supplier, price }
    ...
    const handleAddPrice = (newEntry) => {
        ...
        setSupplierPrices(prev => [...prev, entryWithId]);
    ...
    return (
        <>
                    ...
                    <form onSubmit={handleFormSubmit} className="space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-3">
                                <CustomFormSelect label="Default Warehouse" name="warehouse" options={warehouseOptions} initialValue={formValues.warehouse} onSelect={handleInputChange} />
                        ...
                        <div className="overflow-x-auto pb-3">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <button type="button" onClick={handleOpenPriceModal} className="flex items-center space-x-2 py-2 px-4 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all">
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-slate-100 dark:bg-slate-700/50">
                                        ...
                                        <th className="text-right p-4 text-sm font-semibold text-slate-600 dark:text-slate-200">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {supplierPrices.length > 0 ? (
                                        supplierPrices.map((item) => (
                                            <tr key={item.id} className="border-b border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                                ...
                                                <td className="p-4 text-right">
                                                    <button type="button" onClick={() => handleRemovePrice(item.id)} className="text-red-500 hover:text-red-700 p-1 transition-colors">
                                        ...
                                        <tr>
                                            <td colSpan="3" className="p-8 text-center text-sm text-slate-500 italic">No supplier pricing added yet.</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                        ...
                        <div className="pt-4 flex justify-end space-x-3">
                            <button type="button" onClick={onClose} className="px-6 py-2 text-sm font-medium rounded-md text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 transition-colors">Cancel</button>
                            <button type="submit" className="px-6 py-2 text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 shadow-md">Save Product</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <AddSupplierPriceModal 
                ...
                supplierOptions={supplierOptions} // Pass options here
            />
        </>
    );
}
