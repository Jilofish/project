...
function CustomFormSelect({ label, name, options, initialValue, onSelect, placeholder = "Select an option" }) {
    ...
    return (
        <div className="relative w-full" ref={selectRef}>
            ...
            {isOpen && (
                <ul
                    className="absolute z-10 w-full mt-1 bg-white dark:bg-slate-800 rounded-md shadow-xl border border-slate-300 dark:border-slate-600 max-h-60 overflow-auto"
                    role="listbox"
                >
                    {options.map((option) => (
                        <li
